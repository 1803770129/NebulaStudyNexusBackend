...
describe('Question Data Model Property Tests', () => {
   ...
   */
  const richContentArb = fc.tuple(
    fc.string({ minLength: 1, maxLength: 200 }),
    fc.string({ minLength: 1, maxLength: 200 })
  ).map(([raw, rendered]): RichContent => ({
    raw,
    rendered,
  }));
   ...
   */
  const optionArb = fc.tuple(
    fc.constantFrom('A', 'B', 'C', 'D'),
    richContentArb,
    fc.boolean()
  ).map(([id, content, isCorrect]): Option => ({
    id,
    content,
    isCorrect,
  }));
  ...
  describe('Property 6: Data Model Structure Integrity', () => {
    ...
    it('array of Options should all have valid RichContent', () => {
      fc.assert(
        fc.property(
          fc.array(optionArb, { minLength: 2, maxLength: 4 }),
          (options) => {
