...
describe('ContentService Property Tests', () => {
          ...
          useValue: {
            renderBatch: jest.fn().mockImplementation((formulas) => {
              ...
              for (const { latex, inline } of formulas) {
                ...
                results.set(
                  key,
                  `data:image/svg+xml;base64,mock_${Buffer.from(latex).toString('base64')}`,
                );
              }
  ...
  const latexFormulaArb = fc.oneof(
    ...
    // Power expressions
    fc
      .tuple(fc.constantFrom('x', 'y', 'z'), fc.integer({ min: 2, max: 9 }))
      .map(([v, n]) => `${v}^${n}`),
    // Fractions
    fc
      ...
      .map(([a, b]) => `\\frac{${a}}{${b}}`),
    // Sum notation
    fc.constantFrom('\\sum_{i=1}^{n} i', '\\sum_{k=0}^{n} k^2', '\\int_0^1 x dx'),
    // Common formulas
    fc.constantFrom('E=mc^2', 'a^2+b^2=c^2', '\\pi r^2', '\\sqrt{x}', '\\alpha + \\beta'),
  );

  /**
   * Generator for HTML content with inline formulas
   */
  const htmlWithInlineFormulaArb = fc
    .tuple(
   ...
   */
  const htmlWithBlockFormulaArb = fc
    .tuple(
      fc.string({ minLength: 0, maxLength: 50 }).filter((s) => !s.includes('$')),
      latexFormulaArb,
      fc.string({ minLength: 0, maxLength: 50 }).filter((s) => !s.includes('$')),
  ...
  describe('Property 4: Content Processing Round Trip', () => {
    ...
    it('inline formulas should be replaced with img tags in rendered content', async () => {
      await fc.assert(
        fc.asyncProperty(htmlWithInlineFormulaArb, async (rawContent) => {
          const result = await service.processContent(rawContent);
    ...
    it('block formulas should be replaced with img tags in rendered content', async () => {
      await fc.assert(
        fc.asyncProperty(htmlWithBlockFormulaArb, async (rawContent) => {
          const result = await service.processContent(rawContent);
    ...
    it('content without formulas should have identical raw and rendered', async () => {
      await fc.assert(
        fc.asyncProperty(plainHtmlArb, async (rawContent) => {
          const result = await service.processContent(rawContent);
